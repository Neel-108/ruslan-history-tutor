...
# async def maybe_update_warm_summary(user_id: int, topic: str, checkpoint: str):
    ...
    # """
  # # if not topic or not checkpoint:
    # if not checkpoint:
        # return
    ...
    # if existing_summary:
        # new_summary = f"{existing_summary}; {topic}: {checkpoint}"
    # else:
        # new_summary = f"{topic}: {checkpoint}"
...
async def handle_message(message: Message):
    ...
    # if intent == "ABUSE":
        # await message.answer(ABUSE_RESPONSE)
    ...
    # if intent == "CONTINUE" and hot_state['current_topic']:
        # mode = MODE.CONTINUE
