...
class RuslanLogic:
    ...
    # def parse_topic_year(self, question: str) -> Optional[int]:
        ...
        # for topic, year in self.topic_years.items():
            # if topic in q_lower:
                # print(f"DEBUG: Matched keyword '{topic}' → year {year}")
                # return year
    ...
    # def validate_question(self, question: str, grade: int) -> Tuple[bool, str]:
        # """
        # Combined validation: FGOS rules + chronology.
        
        # Returns:
            # (is_valid: bool, refusal_message: str)
        # """
        # # First check FGOS rules
        # should_refuse, fgos_msg = self.check_fgos_refusal(question, grade)
        # if should_refuse:
            # return (False, fgos_msg)
        
        # # Then check chronology
        # chron_ok, chron_msg = self.check_grade_chronology(question, grade)
        # if not chron_ok:
            # return (False, chron_msg)
